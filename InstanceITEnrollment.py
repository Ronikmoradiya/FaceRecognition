...
window = tk.Tk()
window.title("Hackathon Face Recognition")
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            #clear()
            #clear1()
            #cam = cv2.VideoCapture("rtsp://admin:admin%40@192.168.1.64/axis-media/media.amp")
            ...
            while (True):
                ...
                face_profile_left = detector_profile.detectMultiScale(gray, 1.3, 5)
                flipped = cv2.flip(gray, 1)
                faces_profile_right = detector_profile.detectMultiScale(flipped, 1.3, 5)
                if sampleNum < 41:
                ...
                elif sampleNum>=41 and sampleNum < 80:
                    for (x, y, w, h) in face_profile_left:
                ...
                elif sampleNum>79 or sampleNum==120:    
...
def training():
    ...
    try:
        recognizer.save("InstaceIT_WithSideFaceLabel\Trainner_final.yml")
    except Exception as e:
